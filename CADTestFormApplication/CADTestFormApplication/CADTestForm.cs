...
namespace CADTestFormApplication
{
	...
	{
		private SaveSettings _svSet;
		private SortedList _settingsList;
		private string fileSettingsName;
		CADImage _cadImage;
		...
		//		if (_settingsList.ContainsKey(key))
		//			this.mlng.Path = Convert.ToString(_settingsList[key]);
		//	}
		//	else
		...
		internal void SetSettings()
		{
			...
			if (_settingsList.ContainsKey(key))
				lngFile = (string)_settingsList[key];
			mlng.LoadLNG(lngFile);
			this.Text = mlng.SetLanguage(this.Controls, this.Menu, this.Text);
			//Language ID
			key = ApplicationConstants.languageIDstr;
			if (_settingsList.ContainsKey(key))
				this.curLngIndex = Convert.ToByte(_settingsList[key]);
			//BackgroundColor
			key = ApplicationConstants.backcolorstr;
			tmp = ApplicationConstants.blackstr;
			if (_settingsList.ContainsKey(key))
				tmp = Convert.ToString(_settingsList[key]);
			...
			if (_settingsList.ContainsKey(key))
				cn = Convert.ToInt32(_settingsList[key]);
			//SHXPaths
			for (int i = 0; i < cn; i++)
			{
				key = string.Format("SHXPath_{0}", (i + 1));
				if (_settingsList.ContainsKey(key))
					this.cadEditorControl.SHXForm.AddPath(Convert.ToString(_settingsList[key]));
			}
			//First start
		...
		{
			string tmp;
			_settingsList = new SortedList();
			//Language path
			_settingsList.Add(ApplicationConstants.languagepath, mlng.Path);
			//Language
			_settingsList.Add(ApplicationConstants.languagestr, lngFile);
			//Language ID
			_settingsList.Add(ApplicationConstants.languageIDstr, this.curLngIndex);
			//BackgroundColor
			if (cadEditorControl.EditorCADPictureBox.BackColor == Color.Black)
				tmp = ApplicationConstants.blackstr;
			else tmp = ApplicationConstants.whitestr;
			_settingsList.Add(ApplicationConstants.backcolorstr, tmp);
			//Color drawing
			if (this.cadEditorControl.Image != null)
				_settingsList.Add(ApplicationConstants.colorDraw, (this.cadEditorControl.Image.DrawMode == CADImport.CADDrawMode.Normal));
			else
				_settingsList.Add(ApplicationConstants.colorDraw, this.cadEditorControl.ColorDraw);
			//SHXPathCount
			int cn = cadEditorControl.SHXForm.lstDir.Items.Count;
			_settingsList.Add(ApplicationConstants.shxpathcnstr, cadEditorControl.SHXForm.lstDir.Items.Count);
			//SHXPaths
			for (int i = 0; i < cn; i++)
			{
				_settingsList.Add("SHXPath_" + (i + 1), cadEditorControl.SHXForm.lstDir.Items[i]);
			}
			this.cadEditorControl.NewProtectionSettings(_settingsList);
		}
		...
		{
			CreateNewSettingsList();
			_svSet.SaveOptions(_settingsList);
		}
		...
		private void cadEditorControl_Load(object sender, EventArgs e)
		{
